...
function getUtcDateTime(): DateTime {
  // FortyGuard forecast data can lag behind the current hour. Use the previous hour.
...
async function fortyGuardRequest(path: string, init: RequestInit = {}) {
  ...
  try {
    ...
    if (!response.ok || payload?.error === true || payload?.error === 'true') {
      throw new FortyGuardHttpError(response.status, path, String(payload?.message ?? payload?.error ?? response.statusText ?? 'FortyGuard request failed'), text.slice(0, 2000));
    }
    return payload;
  } finally {
...
function numericArray(value: unknown): (number | null)[] {
  ...
  return value.map((item) => {
    if (item === null || item === undefined || Number(item) === -999) return null;
...
function extractHeatmapTemperature(result: any): number | null {
  ...
  const candidates = [
    stats?.Temperature_stats?.Mean, stats?.Temperature_stats?.mean,
    stats?.temperature_stats?.Mean, stats?.temperature_stats?.mean,
    stats?.mean_temperature, stats?.meanTemperature,
    stats?.average_temperature, stats?.averageTemperature,
  ];
  ...
  const distributionValues = numericArray(distribution).filter((value): value is number => value !== null);
  if (distributionValues.length) return mean(distributionValues);
  return null;
}
...
async function runHeatmap(coordinates: Coordinates) {
  ...
  const submitted = await fortyGuardRequest('/v1/heatmap', {
    method: 'POST',
    body: JSON.stringify({ polygon_aoi: buildPolygon(coordinates), date_time: { start_date: dateTime.startDate, start_time: dateTime.startTime, filter_type: 1 }, granularity: 100 }),
  });
  ...
  return {
    temperature, activityId, dateTime, recordedAt: new Date().toISOString(),
    diagnostics: { resultKeys: Object.keys(result ?? {}), statsKeys: Object.keys(stats), nCells: Number(stats.n_cells ?? 0), featuresCount: features.length },
  };
}
...
async function runEnvironmentalTrend(coordinates: Coordinates, temperature: number, date: string) {
  const submitted = await fortyGuardRequest('/v1/env_params', {
    ...
    body: JSON.stringify({
      ...
      analysis: ['apparent_temperature_celsius', 'heat_index_celsius', 'relative_humidity_percent'],
    }),
  });
  const activityId = submitted?.data?.activity_id;
  ...
  const points = Array.from({ length }, (_, index) => ({
    ...
    temperature: apparent[index] ?? heatIndex[index] ?? null,
